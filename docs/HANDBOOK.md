...
> hand-coded dribbler *loses* to trained strikers — **0% win** over 64 games (no own-goals to exploit) —
> and a trained striker wins **outright**). Kids earn NeuroLab by feeling exactly where rules run out — then open it
...
- **Soccer — the same brain, a new game.** A walled pitch with a goal at each end and a ball in the middle: step on the ball to **shove it one tile**, dribble it into the opponent's net to score, and a live scoreline counts the goals in a timed match. **You can't score in your own net:** shove the ball toward the net you defend and your robot **and the ball trade places and you spin 180°** — *automatically* — so the ball ends up ahead of you pointing the right way and **own-goals are impossible**. You can also trigger that same **`zap`-swap** yourself (zap while facing the ball) to flip it around on demand. Each goal mouth wears its **defender's** colour, so you score in the *other* colour (the opponent's end, like real soccer). It's the *same* `10 → 8 → 5` brain — it just senses the **ball** as its objective and the **goal** and **rival** as bearings — so you can **Teach / Q-Learn / Evolve** a soccer bot, or **hand-code** one with the `ball-ahead` / `net-left` sense blocks (and **and/or** conditions, e.g. *if ball-ahead **and** net-left*). A pre-trained **house team** (Strika, Dribbla, Volley, Nutmeg, Boots) gives a beatable first match. Great for the "one brain, many jobs" point: a maze brain, a fighter, and a striker are the **same network shape** trained on different objectives.
