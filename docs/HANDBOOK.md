...
- **Soccer — the same brain, a new game.** A walled pitch with a goal at each end and a ball in the middle: step on the ball to **shove it one tile**, dribble it into the opponent's net to score, and a live scoreline counts the goals in a timed match. There's also a **`zap`-swap**: zap while facing the ball and your robot and the ball **trade places** — a one-move way to get on the ball's far side and turn it back toward goal when caught on the wrong one (the slow alternative, circling around, is exactly how a bot shoves it into its *own* net). Each goal mouth wears its **defender's** colour, so you score in the *other* colour (the opponent's end, like real soccer) and an own-goal is easy to spot — and the soccer trainer now **penalises a wrong-way push 2×** and **rewards a goalward zap**, so a trained brain learns to avoid own-goals and use the swap. It's the *same* `10 → 8 → 5` brain — it just senses the **ball** as its objective and the **goal** and **rival** as bearings — so you can **Teach / Q-Learn / Evolve** a soccer bot, or **hand-code** one with the `ball-ahead` / `net-left` sense blocks (and **and/or** conditions, e.g. *if ball-ahead **and** net-left*). A pre-trained **house team** (Strika, Dribbla, Volley, Nutmeg, Boots) gives a beatable first match. Great for the "one brain, many jobs" point: a maze brain, a fighter, and a striker are the **same network shape** trained on different objectives.
